...
def Complete(image):
    ...
    a = numpy.asarray(im) # a is the matrix representation of the original image

    b = len(a) - 1 # b is used to calculate how many mini-matrix a has
    ...
    def SymCal(a): #function to calculate symmetry of 1 matrix and return the total score
        ...
        SymList = [0,0,0,0,0,0,0,0,0] #SymList contains the no. of same pixel to reflected/rotated images
...
##Comparison = [[] for i in range(45)]

##ImageName = [[] for i in range(45)]
...
# compare to the research paper, we are not just saying yes and no, we're calculating the exact number of pixels that say the same 
